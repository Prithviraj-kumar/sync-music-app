...
import { LogOut, Play, Pause, SkipForward, SkipBack, Plus, Mic, MicOff, Volume2, VolumeX, Headphones, Crown, Star, MessageCircle, Users, Music2, X } from 'lucide-react';
...
function ListeningRoom() {
  ...
  useEffect(() => {
    ...
    newSocket.on('song-changed', (data) => {
      ...
      if (audioRef.current && data.song) audioRef.current.currentTime = data.currentTime;
    });
  ...
  const handleFileUpload = async (e) => {
    ...
    finally { setUploading(false); setShowAddModal(false); }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-950">
      ...
      <div className="fixed top-0 left-0 right-0 bg-black/80 backdrop-blur-lg z-30 p-3 flex justify-between items-center border-b border-white/10">
        <div>
          <h2 className="text-base font-bold text-white">🎵 Music Party Room</h2>
          <p className="text-[10px] text-slate-400">Room ID - {roomId}</p>
        </div>
        <div className="flex gap-2">
          {isMobile && (
            <button onClick={() => setShowChatMobile(!showChatMobile)} className="p-2 rounded-full bg-white/10">
      ...
      <div className="pt-16 flex flex-col md:flex-row h-screen overflow-hidden">
        ...
        {/* LEFT COLUMN - Playlist (70% on desktop) */}
        <div className="flex-1 md:flex-[2] overflow-y-auto p-4">
          ...
          <div className="bg-white/5 rounded-2xl p-4 mb-6">
            ...
            <div className="flex items-center justify-center gap-2 mt-3">
              ...
              <input type="range" min="0" max="1" step="0.01" value={volume} onChange={handleVolumeChange} className="w-24 h-1 rounded-full bg-slate-700" />
            </div>
          </div>

          {/* Add Music Button */}
          {(role === 'host' || role === 'co-host') && (
            <button onClick={() => setShowAddModal(true)} className="w-full py-3 rounded-xl bg-primary/20 text-primary font-semibold flex items-center justify-center gap-2 border border-primary/30 mb-6">
              <Plus className="w-5 h-5" /> Add Music
            </button>
          )}

          {/* PLAYLIST Section - Like Screenshot */}
          <div>
            <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
              <Music2 className="w-5 h-5" /> PLAYLIST
            </h3>
            <div className="space-y-2">
              {queue.length === 0 ? (
                <div className="text-center py-12 bg-white/5 rounded-xl">
                  <Music2 className="w-12 h-12 text-slate-600 mx-auto mb-2" />
                  <p className="text-slate-400">No songs in playlist</p>
                </div>
              ) : (
                queue.map((song, idx) => (
                  <div key={idx} className="bg-white/5 rounded-xl p-3 hover:bg-white/10 transition-all">
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-slate-400 mt-1">#{idx + 1}</p>
                      </div>
                      <div className="flex gap-2">
                        {(role === 'host' || role === 'co-host') ? (
                          <>
        ...
        {/* RIGHT COLUMN - Chat (30% on desktop) */}
        <div className={`${isMobile ? (showChatMobile ? 'fixed inset-0 z-50 bg-black' : 'hidden') : 'w-80'} bg-slate-900/50 backdrop-blur-sm border-l border-white/10 flex flex-col h-full`}>
          ...
          <div className="p-4 border-b border-white/10">
            <div className="flex justify-between items-center">
              ...
              {isMobile && (
                <button onClick={() => setShowChatMobile(false)} className="p-2 rounded-full bg-white/10">
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {participants.map((p) => (
                <div key={p.id} className="flex items-center gap-1 px-2 py-1 rounded-full bg-white/10">
                  ...
                  {p.id === userId && <span className="text-[10px] text-slate-400 ml-1">(You)</span>}
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="p-4 border-t border-white/10">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2 rounded-xl bg-white/10 text-white placeholder:text-slate-500 text-sm border border-white/10 focus:outline-none focus:border-primary"
              />
      ...
      {/* Add Music Modal */}
      {showAddModal && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4" onClick={() => setShowAddModal(false)}>
          <div className="bg-slate-900 rounded-2xl p-6 w-full max-w-sm border border-white/10" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setShowAddModal(false)} className="p-1 rounded-full bg-white/10">
                <X className="w-5 h-5 text-white" />
              </button>
            </div>
      ...
      {isMobile && currentSong && (
        <div className="fixed bottom-0 left-0 right-0 bg-black/95 backdrop-blur-lg p-2 border-t border-white/10 z-20">
          <div className="flex items-center gap-2">
            ...
            <button onClick={handlePlayPause} className="p-2 rounded-full bg-primary">
