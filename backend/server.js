...
io.on('connection', (socket) => {
  ...
  // FIXED: Join room with proper host/co-host assignment
  socket.on('join-room', ({ roomId, username }) => {
    ...
    console.log(`Current host: ${room.host}`);
    console.log(`Current co-host: ${room.coHost}`);
    ...
    else if (room.participants.length === 1 && room.coHost === null) {
      role = 'co-host';
  ...
  // Voice activity (for auto volume reduction)
  socket.on('voice-activity', ({ isSpeaking }) => {
    ...
    if (room) {
      ...
      if (participant) {
        ...
        // Check if anyone is speaking to adjust volume
